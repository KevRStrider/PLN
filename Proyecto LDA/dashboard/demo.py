...
import pandas as pd
import numpy as np
from getdatafromcsv import getData, getData2
...
data, weeks, topics, promedios = getData2()
#print(weeks)
#print(data)

fig = createEmptyFig("Seleccione un topico", colors["background"])
...
for x in data.keys():
    probs["Tema"].append(x)
    probs["Probabilidad"].append(promedios[x])
fig3 = px.bar(probs, x="Tema", y="Probabilidad")
fig3.update_layout(
...
app.layout = html.Div(style={'backgroundColor': colors['background'], "margin-top": "0"}, children=[
    ...
    html.H3(children = ["Metodología"],
            style = {"color": colors["text"]},
            className = "ml-3"),
    html.P("""Una vez preprocesados los textos, agrupamos las palabras contiguas en cadenas únicas. Luego, aplicamos el algoritmo TF-IDF a estos datos para determinar la relevancia de las palabras en un documento, eliminando así las palabras poco relevantes y frecuentes.

Después de aplicar el algoritmo, entrenamos el modelo LDA con nuestros datos, especificando el número de tópicos (K) deseados. El modelo nos devuelve dicha cantidad de tópicos junto con las palabras asociadas y la probabilidad de pertenencia a cada tópico. Utilizando esta información, asignamos un nombre a cada tópico.""",
           style={"color": colors["text"]},
           className = "ml-3"),
    ...
    children = [
        ...
        html.Div(
            ...
            dcc.Dropdown(
                ...
                id = "input2",
                placeholder="Selecciona un tema",
                className = "col-sm float-right mt-5 w-50"
            ),
            ]
        )]
    ),
...
dcc.Graph(
    ...
    html.H4(children = ["Promedio de probabilidad"],
        style = {"color": colors["text"]},
        className = "ml-3 mt-3 mb-4"),
        dcc.Graph(
        id='graph3',
